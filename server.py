...
FAST_MCAP  = "hackathon_fast_laps.mcap"
GOOD_MCAP  = "hackathon_good_lap.mcap"
STATE_TOPIC  = "/constructor0/state_estimation"
CAMERA_TOPIC = "/constructor0/sensor/camera_fl/compressed_image"
CALIB_PATH   = "intrinsics/camera_fl_info.yaml"
TRACK_BOUNDS = "yas_marina_bnd.json"

MAP_SIZE  = 300   # pixels square (circle diameter)
MAP_PAD   = 10    # padding inside the circle

BRAKE_UP   = 0.35   # fast attack  (large value = snappier)
BRAKE_DOWN = 0.12   # slow release (small value = smoother fade)
...
def load_trajectories():
    if not os.path.exists(FAST_NPY):
        data = rip_trajectory(FAST_MCAP, STATE_TOPIC)
        if data is not None:
            np.save(FAST_NPY, data)

    if not os.path.exists(GOOD_NPY):
        data = rip_trajectory(GOOD_MCAP, STATE_TOPIC)
        if data is not None:
            np.save(GOOD_NPY, data)

    try:
        fast_data = np.load(FAST_NPY)
        good_data = np.load(GOOD_NPY)
    except FileNotFoundError:
        print("ERROR: trajectory .npy files missing.")
        return None, None

    return fast_data, good_data


def load_camera_frames():
    camera_frames = rip_camera_frames(FAST_MCAP, CAMERA_TOPIC)
...
def _classify_turn(curv):
    for label, color, warn_dist, min_c in TURN_CATS:
        if curv >= min_c:
            return label, color, warn_dist
    return "BEND", (0, 210, 255), 70


def detect_turns(good_data):
    from scipy.ndimage import uniform_filter1d
    from scipy.signal import find_peaks

    _gx, _gy = good_data[:, 0], good_data[:, 1]
    _gh = np.arctan2(np.gradient(_gy), np.gradient(_gx))
    _gc = np.abs(_angle_diff(np.roll(_gh, 1), _gh))
    _gdx = np.diff(_gx, append=_gx[-1])
    _gdy = np.diff(_gy, append=_gy[-1])
    ...
    for idx in _apex_idxs:
        ...
        turns.append({
            ...
            "color":     color,
            "warn_dist": warn_dist,
            "ref_speed": float(good_data[idx, 4]),
        })

    print(f"Detected {len(turns)} turns: "
          + ", ".join(f"{t['label']}@{t['cum_dist']:.0f}m" for t in turns[:6]) + " ...")

    return turns, _gcum


def _nearest_upcoming_turn(car_x, car_y, good_tree, gcum, turns, lookahead_m=300.0):
    _, near_idx = good_tree.query([car_x, car_y])
    car_dist = gcum[near_idx]
    lap_len  = gcum[-1]
    best = None
...
def render_hud(frame, speed_ms, gas, brake_norm, result):
    h, w = frame.shape[:2]
    ...
    for si in range(num_segs):
        sx = bx0 + si * seg_step
        if si < filled_segs:
            seg_col = _lerp_color(SPD_STOPS, si / max(1, num_segs - 1))
...
def render_minimap(frame, map_base_frame, car_x, car_y, car_yaw, world_to_map):
    h, w = frame.shape[:2]
    mx, my = 14, h - MAP_SIZE - 14
    cx_m = cy_m = MAP_SIZE // 2
    r_m = MAP_SIZE // 2 - 2

    cp = world_to_map(car_x, car_y)
    ...
    rotated = cv2.warpAffine(map_base_frame, rot_mat, (MAP_SIZE, MAP_SIZE),
...
def build_scene_entities(fast_data, good_data, track_boundaries, ts, car_x, car_y, car_z, car_yaw):
    entities = [
        SceneEntity(
...
def publish_frame(
    i, now_ns, ts,
    fast_data, good_data, track_boundaries,
    camera_frames, calib, K, dist,
    good_tree, turns, gcum,
    map_base, world_to_map,
    brake_ema,
    spoken_turns, voice_lock,
    camera_chan, camera_info_chan, scene_chan, tf_chan, ann_chan,
):
    car_x, car_y, car_z, car_yaw = (
        fast_data[i, 0], fast_data[i, 1], fast_data[i, 2], fast_data[i, 3],
    )

    map_base_frame = map_base.copy()
    for t in turns:
        if t["label"] == "SHARP":
            tc = t["color"]
            ap = world_to_map(good_data[t["idx"], 0], good_data[t["idx"], 1])
            cv2.circle(map_base_frame, ap, 4, (int(tc[0]), int(tc[1]), int(tc[2])), -1)
    ...
    result = _nearest_upcoming_turn(car_x, car_y, good_tree, gcum, turns)
    in_warn_zone = (result and result[1] <= result[0]["warn_dist"]
                    and result[0]["label"] == "SHARP")
    ...
    if camera_frames:
        ...
        in_range_sorted = np.sort(in_range)
        ...
        alpha = BRAKE_UP if brake_raw_norm > brake_ema[0] else BRAKE_DOWN
        brake_ema[0] = alpha * brake_raw_norm + (1.0 - alpha) * brake_ema[0]
        brake_norm = brake_ema[0]

        if in_warn_zone:
            t_key = result[0]["idx"]
            if t_key not in spoken_turns:
                spoken_turns.add(t_key)
                label_v = result[0]["label"].lower()
                dist_rounded = int(round(result[1] / 10) * 10)
                _speak(f"{label_v} turn in {dist_rounded} meters", voice_lock)

        frame = render_hud(frame, speed_ms, gas, brake_norm, result)
        frame = render_minimap(frame, map_base_frame, car_x, car_y, car_yaw, world_to_map)
        ...
        if pts2d is not None and len(pts2d) >= 2:
            ...
            if len(pts2d_valid) >= 2:
                ...
                line_pts = pts2d_valid[::2]
                if brake_norm < 0.15:
                    dot_r, dot_g, dot_b = 0.0, 1.0, 0.3
                elif brake_norm < 0.5:
                    dot_r, dot_g, dot_b = 1.0, 0.55, 0.0
                else:
                    dot_r, dot_g, dot_b = 1.0, 0.1, 0.1
                ann_chan.log(ImageAnnotations(
                    ...
                    points=[
                        PointsAnnotation(
                            timestamp=ts,
                            type=PointsAnnotationType.LineStrip,
                            points=[Point2(x=float(p[0]), y=float(p[1])) for p in line_pts],
    ...
    entities = build_scene_entities(fast_data, good_data, track_boundaries, ts, car_x, car_y, car_z, car_yaw)
...
def main():
    fast_data, good_data = load_trajectories()
    if fast_data is None or good_data is None:
        return

    good_tree = KDTree(good_data[:, :2])

    with open(TRACK_BOUNDS) as f:
    ...
    map_base, world_to_map = build_minimap(good_data, fast_data)
    turns, gcum = detect_turns(good_data)
